...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $name = trim(mysqli_real_escape_string($db,htmlspecialchars($_POST['ImageImageName'])));
    ...
    if ((!empty($year) and  preg_match('/[^0-9]/', $year)) or (!empty($year) and  (strlen($year) != 4))) {
        ...
        if ($ISBN != $_POST['ISBN'] or $title != $_POST['Title'] or $authors != $_POST['Authors'] or $year != $_POST['Year'] or $bookcondition != $_POST['BookCondition']) {
            ...
            if ($_SESSION['edit_book']) {
                ...
                $query = "UPDATE Books SET  ISBN = '$ISBN', Title = '$title', Authors = '$authors', Year = '$year', BookCondition = '$bookcondition', Category = '$category', ImageName = '$name', Image = '$mage' WHERE BookID = '$bookID'";
            }
            else {
...
?>
  <div id="results" onload="center_results();">
