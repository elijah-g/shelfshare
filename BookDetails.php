...
// Set this test to the number 1 - 4 to activate each of the 3 tests on this page.
$testmode = 0;
// 1 tests redirecting unlogged in users to the log in page upon arrival. This test logs you out.
// 2 tests displaying dummy data from the database editing. Book ID 1 holds the dummy data.
// 3 tests the regex filter for the year. 
// 4 tests the rating fetching function.
...
// test 2, to see if the following variables get displayed. Dummy data is 1's
if ($testmode == 2) {
...
if ($testmode == 2) {
    ...
    if ($ISBN != '1' or $title != '1' or $authors != '1' or $year != '2000' or $bookcondition != '1') {
        echo "test 2 failed, ";
    }
    } 
}
    ...
    if ((!empty($year) and  preg_match('/[^0-9]/', $year)) or (!empty($year) and  (strlen($year) != 4))) {
        ...
        if ($ISBN != $_POST['ISBN'] or $title != $_POST['Title'] or $authors != $_POST['Authors'] or $year != $_POST['Year'] or $bookcondition != $_POST['BookCondition']) {
            ...
            if ($_SESSION['edit_book']) {
                ...
                // grab the Owner's ID from the global for the foreign key 
                $ownerID = $_SESSION['userID'];
                // Build the INSERT INTO query for creating books.
                $query = "INSERT INTO Books (ISBN, Title, Authors, Year, BookCondition, OwnerID, Category, ImageName, Image) VALUES ('$ISBN', '$title', '$authors', '$year', '$bookcondition', '$ownerID', '$category', '$name', '$image')";
            }

            // Compare entered book details to isbdnb database
            if (1 != 1) {
                if (empty($error)) {
                    $error = "That does not look like a real book. Please check that it exists in isbndb.";
                }
            }
            else {
                ...
                if (empty($title) or empty($authors)) {
                    ...
                    if ($result) {
                        header("Location: MatchResults.php?BookID={$bookID}");
...
?>
  ...
  <div id="results">
    ...
    <div class="input-group mb-3">
      <input type="text" id="searchbar" class="form-control" placeholder="Lend a book...">
      ...
      success: function(data){
       console.log(data);
        ...
        for(i = 0; i < 10; i++){
            ...
            <input class="mb-2 mr-sm-2" type="file" name="Image" id="image">

            
            <label class="mr-sm-2">Book Condition: </label>
            <input class="form-control mb-2 mr-sm-2" type="text" name="BookCondition" class="form-control">
            ...
            select_div.className = "col"
            select_button = document.createElement("button")
            select_button.className = "btn btn-success"
            select_button.setAttribute("data-toggle", "collapse")
            select_button.setAttribute("data-target", "#collapse-" + booki)
            select_button.innerHTML = "Select"
            select_div.appendChild(select_button)
            
            
            row2.appendChild(isbn_div)
            row2.appendChild(author_div)
            row2.appendChild(year_div)
            row2.appendChild(select_button)
            
            var row3 = document.createElement("div")
            row3.className = "collapse row collapse-form"
            row3.setAttribute("id", "collapse-" + booki)
